...
if len(sys.argv) < 2:
    print("Usage: python spca_nlm.py <image_name>")
    sys.exit(1)
...
model = SparsePCA(n_components=25, alpha=1, max_iter=1000, tol=0.001) 
# Aplica PCA em dataset
spca = model.fit(conjunto_patches)

for h in lista_h:
    print('h = %d' %h)
    # Mede tempo de processamento
    inicio = time.time()
    # Loop principal do SPCA-NLM
    print('SPCA-NLM processing...')
    for i in range(m):
        #if i % 10 == 0:
            #print(i, end=' ')
        sys.stdout.flush()
        for j in range(n):
            ...
            for r in range(rmin, rmax):
                for s in range(smin, smax):
                    ...
                    k = k + 1
            # Aplica SPCA em dataset
            dados_pca = spca.transform(dataset)                
    ...
    # print()
    # Salva arquivo
    arquivo = 'output/NLMSPCA/' + img_name[11:-5] + '_h_' + str(h) + '.png'
    skimage.io.imsave(arquivo, output)

csv_name = 'scikit_spcanlm_results'+ str(sigma) +'.csv'
